...
def read_one_xyz(filename):
    ...
    """
    xyz = []
    with open(filename+"/cp8.xyz", 'r') as f:
        content = f.read()
        ...
        # 开始写入整理完的数据csv
        with open(filename + "/ThreeCC.csv", 'w',
...
def creatnodelist(filepath):
    ...
    # 读csv文件
    numberRow = pd.read_csv(filepath + "/ThreeCC.csv")
    # 定义存放图的边的列表
    # listEdge = []
    # 返回长度
    lengthCsv = len(numberRow)
    with open("/Users/wuhao/PycharmProjects/BaiduMove/百度迁徙数据/01-爬虫-爬取百度迁徙数据/biologicalnetworks/biologicaldata/[3]BLOCKCNT/(c) BTCOCNT-cp8/threeCCnodelist.csv", 'w', encoding="utf-8", newline='') as csvfile:
        writer = csv.DictWriter(csvfile, field_order_move_in)
        writer.writeheader()
...
# creatnodelist("/Users/wuhao/PycharmProjects/BaiduMove/百度迁徙数据/01-爬虫-爬取百度迁徙数据/biologicalnetworks/biologicaldata/[3]BLOCKCNT/(c) BTCOCNT-cp8")



datarow = pd.read_csv("/Users/wuhao/PycharmProjects/BaiduMove/百度迁徙数据/01-爬虫-爬取百度迁徙数据/biologicalnetworks/biologicaldata/[3]BLOCKCNT/(c) BTCOCNT-cp8/threeCCnodelist.csv")
G = nx.Graph()
...
# print("平均度为：", avergerDegree/38100)
# print("点数题目已给出")
