...
# 需要删除的节点
delcityName = ['澳门', '克拉玛依', '白沙黎族自治县', '文昌', '东方', '吴忠', '和田地区', '黄南藏族自治州', '迪庆藏族自治州', '昌江黎族自治县', '乌鲁木齐', '陵水黎族自治县', '阿克苏地区', '保亭黎族苗族自治县', '北屯', '伊犁哈萨克自治州', '五指山', '阿拉善盟', '吉林市', '浙江', '林芝地区', '儋州', '阿拉尔', '吐鲁番地区', '昌都地区', '定安', '克孜勒苏柯尔克孜自治州', '日喀则地区', '哈密地区', '莱芜', '果洛藏族自治州', '山南地区', '毕节地区', '嘉峪关', '玉树藏族自治州', '阿里地区', '琼中黎族苗族自治县', '海口', '那曲地区', '固原', '铁门关', '新疆维吾尔自治区', '阿勒泰地区', '怒江傈僳族自治州', '石嘴山', '万宁', '七台河', '广西壮族自治区', '五家渠', '酒泉', '石河子', '内蒙古自治区', '宁夏回族自治区', '可克达拉', '拉萨', '琼海', '神农架林区', '图木舒克', '昆玉', '乐东黎族自治县', '西藏自治区', '甘南藏族自治州', '三亚', '双河', '昌吉回族自治州', '博尔塔拉蒙古自治州', '大兴安岭地区', '塔城地区', '中卫', '巴音郭楞蒙古自治州', '海东地区', '香港', '银川', '屯昌', '铜仁地区', '澄迈', '喀什地区', '临高', '黄山','澄迈县','临高县',"吐鲁番",'山南',"那曲","日喀则"]

# 需要去除某几个城市
dealdatetwo =['双鸭山','海南藏族自治州','海北藏族自治州']
# 获取时间列表
...
def deal_four_threshold_and_deal_city_name(beginTime,endTime):
    ...
    for j in range(len(dayList)):
        ...
        print("第一步：：" + dayList[j], "这天开始时间为", nowTimebegin)
        # 迁入数据
        try:
            ...
            for cityNameIn_i in range(len(dataWantdelColIn)):
                indexColMoveIn = dataWantdelColIn.loc[cityNameIn_i]
                if indexColMoveIn[0] in delcityName or indexColMoveIn[1] in delcityName or indexColMoveIn[2] < 0.04:
                    dataWantdelColIn = dataWantdelColIn.drop(cityNameIn_i)
            dataWantdelColIn.to_csv(fileName + "01去掉不用的点和小于0.04阈值\\in\\" + dayList[j] + "delNodeCityFour.csv", index=False, encoding="utf-8-sig")
            ...
            try:
                ...
                for cityName_Out in range(len(dataWantdelColOut)):
                    indexColMoveOut = dataWantdelColOut.loc[cityName_Out]
                    if indexColMoveOut[0] in delcityName or indexColMoveOut[1] in delcityName or indexColMoveOut[2] < 0.04:
                        dataWantdelColOut = dataWantdelColOut.drop(cityName_Out)
                dataWantdelColOut.to_csv(fileName + "01去掉不用的点和小于0.04阈值\\out\\" + dayList[j] + "delNodeCityFour.csv",
...
def deal_sameCol_in_Alonecsv(beginTime,endTime):
    ...
    for i in range(len(dayList)):
        nowTimebegin = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        print("第二步：：" + dayList[i], "这天开始时间为", nowTimebegin)
        # 迁入数据
        try:
            moveIn = pd.read_csv(fileName + "01去掉不用的点和小于0.04阈值\\in\\" + dayList[i] + "delNodeCityFour.csv")
        except Exception as problem:
            ...
            with open(fileName + "02单独的In和out进行相同行合并\\in\\" + dayList[i] + "delNodeCityFour_Alone.csv", 'w',
                ...
                # 提高运算效率。简化数据，指标
                index_data = 0
                for moveIn_city_i in range(0,moveIn["city_name"].count()):
                    # 使用列表比较是否相同move_in
                    listMoveIn_one = []
                    indexColMoveIn_one = moveIn.loc[moveIn_city_i]
                    listMoveIn_one.append(indexColMoveIn_one[0])
                    listMoveIn_one.append(indexColMoveIn_one[1])
                    # move_out 每一行
                    for moveIn_city_j in range(moveIn_city_i+1,moveIn["city_name"].count()):
                        # 使用列表比较是否相同move_out
                        listMoveIn_two = []
                        indexColMoveIn_two = moveIn.loc[moveIn_city_j]
                        listMoveIn_two.append(indexColMoveIn_two[0])
                        listMoveIn_two.append(indexColMoveIn_two[1])
                        # 获取到索引
                        ...
                        if compare_two_str(listMoveIn_one, listMoveIn_two):
                            valueColThree = (indexColMoveIn_one[2] + indexColMoveIn_two[2]) / 2
                            row = {"city_name": listMoveIn_one[0], "city_id_name": listMoveIn_one[1],
                                   "num": valueColThree}
                            writer.writerow(row)
            csvfile.close()
            ...
            # 迁出数据
            try:
                moveOut = pd.read_csv(fileName + "01去掉不用的点和小于0.04阈值\\out\\" + dayList[i] + "delNodeCityFour.csv")
            except Exception as problem:
            ...
            # 开始写入out数据
                ...
                with open(fileName + "02单独的In和out进行相同行合并\\out\\" + dayList[i] + "delNodeCityFour_Alone.csv", 'w',
                    ...
                    writer.writeheader()
                    # move_in 每一行
                    for moveOut_city_i in range(0, moveOut["city_name"].count()):
                        # 使用列表比较是否相同move_in
                        listMoveOut_one = []
                        indexColMoveOut_one = moveOut.loc[moveOut_city_i]
                        listMoveOut_one.append(indexColMoveOut_one[0])
                        listMoveOut_one.append(indexColMoveOut_one[1])
                        # move_out 每一行
                        for moveOut_city_j in range(moveOut_city_i + 1, moveOut["city_name"].count()):
                            # 使用列表比较是否相同move_out
                            listMoveOut_two = []
                            indexColMoveOut_two = moveOut.loc[moveOut_city_j]
                            listMoveOut_two.append(indexColMoveOut_two[0])
                            listMoveOut_two.append(indexColMoveOut_two[1])
                            # 判断两个列表是否相同 ，来进value值相加除二
                            if compare_two_str(listMoveOut_one, listMoveOut_two):
                                valueColThree = (indexColMoveOut_one[2] + indexColMoveOut_two[2]) / 2
                                row = {"city_name": listMoveOut_two[0], "city_id_name": listMoveOut_two[1],
                                       "num": valueColThree}
                                writer.writerow(row)
                csvfile.close()
            nowTimeEnd = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
...
# deal_four_threshold_and_deal_city_name(20200102,20211028)
#
# deal_sameCol_in_Alonecsv(20200102,20211028)
#
# merge_inAndout_final_data(20200102,20211028)
