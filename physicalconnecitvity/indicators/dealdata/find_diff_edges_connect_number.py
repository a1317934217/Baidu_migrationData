...
['0205_0206data_risiing.csv', '0207_0208data_risiing.csv', '0208_0209data_falling.csv', '0209_0210data_risiing.csv', '0210_0211data_falling.csv', '0212_0213data_falling.csv', '0221_0222data_risiing.csv', '0223_0224data_risiing.csv', '0306_0307data_risiing.csv', '0315_0316data_risiing.csv', '__init__.py']
remove_edge_list_diffedge =['0101_0102data_falling.csv','0104_0105data_falling.csv','0111_0112data_falling.csv','0121_0122data_falling.csv']
remove_edge_list_origionnet = ["20200101finalData.csv","20200104finalData.csv","20200111finalData.csv","20200121finalData.csv"]

add_edge_list_diffedge = ['0102_0103data_risiing.csv','0103_0104data_risiing.csv','0107_0108data_risiing.csv','0110_0111data_risiing.csv','0120_0121data_risiing.csv']
add_edge_list_origionnet =["20200102finalData.csv","20200103finalData.csv","20200107finalData.csv","20200110finalData.csv","20200120finalData.csv"]


def remove_edgeTo_caculate(listDiffedge,listOrigionnet):
    for list_diffedge,list_origionnet in zip(listDiffedge,listOrigionnet):
        dataMiga = pd.read_csv(filepath_front + list_diffedge)
        list_edge_impact={}
        for row in dataMiga.itertuples():
            city_name = getattr(row, "city_name")
            city_id_name = getattr(row, "city_id_name")
            G = drawpicture(fileNamePath_one+list_origionnet)
            G.remove_edge(city_name,city_id_name)
            new_average_connectiy = nx.average_node_connectivity(G)
            list_edge_impact[city_name+"-"+city_id_name] = new_average_connectiy
        name = ["edges", 'indicator_value']
        finall_indicators = pd.DataFrame(columns=name, data= sorted(list_edge_impact.items(),key = lambda x:x[1],reverse = True))  # 数据有三列，列名分别为one,two,three
        finall_indicators.to_csv(filepath_front + list_diffedge[:21] + "_finall_indicators.csv", index=False, header=name,
                                 encoding="utf-8")
        print(list_edge_impact)

def addd_edgeTo_caculate(listDiffedge,listOrigionnet):
    for list_diffedge,list_origionnet in zip(listDiffedge,listOrigionnet):
        dataMiga = pd.read_csv(filepath_front + list_diffedge)
        list_edge_impact={}
        for row in dataMiga.itertuples():
...
# remove_edgeTo_caculate(remove_edge_list_diffedge,remove_edge_list_origionnet)
# addd_edgeTo_caculate(add_edge_list_diffedge,add_edge_list_origionnet)
