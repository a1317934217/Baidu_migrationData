...
# 计算点连通性
def nodecconnectivity(allFilePath, begindata, enddata):
    """
    返回绘制图表的
    X轴：日期
    Y轴：点连通性
    ...
    for i in range(len(listData20_21)):
        ...
        # print(listData20_21[i], "这天的点连通性列表为：", listNodeConnectivityEveryDay)
    # 排序 为了取第一个值
    listNodeConnectivityEveryDay.sort()
    # Y轴最终的数值
    listNodeConnectivity.append(listNodeConnectivityEveryDay[0])
    # 画图 设置X轴显示效果
    fig = plt.figure()
    ...
    plt.title('百度迁徙2020-2021点连通性折线图', FontProperties=font)
    ax.plot(listXdata, listNodeConnectivityEveryDay)
    plt.show()

nodecconnectivity("/Users/wuhao/百度迁徙数据爬取/物理联通指标所需数据/", 20200101, 20211028)
