...
def fetch_one_loc(level, move_in, location_id, date):
    ...
    if dtype == 'country':
        ...
        print(" dtype == 'country':  final_result is ====",final_result)
    ...
    while True:
        try:
            ...
            print(" the last  final_result is ====", final_result)
    ...
    for code, loc in tqdm(loc_id_dict.items()):
        ...
        for level in ['city', 'province']:
            for move_in in [True, False]:
                #缺少数据！！！
                #print("level is =====", level, "move_in is =====", move_in,"code is =====", code,"date is =====",date)
...
def fetch_timerange(start, end):
    ...
    while start <= end:
        ...
        print("province_result===============================================" ,province_result)
        print("city_result===============================================" ,city_result)
        ...
        print("cn_res_c_i===============================================" ,cn_res_c_i)
        print("cn_res_c_o===============================================" ,cn_res_c_o)
        # print("cn_res_p_i===============================================" ,cn_res_p_i)
        # print("cn_res_p_o===============================================" ,cn_res_p_o)
        start += datetime.timedelta(days=1)


        #城市流入数据
        for i in cn_res_c_i:
            excelName = cn_res_c_i['date']
            excelInOrOut = cn_res_c_i['date']
            lastNumber = cn_res_c_i['distribution']
            data = pd.DataFrame(lastNumber[1:-1])
            writer = pd.ExcelWriter(excelName + '.xls')
            # 写入Excel文件
...
if __name__ == '__main__':
    # 省级包括直辖市和港澳，不包括台湾
    # 市级也包括直辖市和港澳，不包括台湾
    fetch_timerange('20210701', '20210701')
    #fetch_timerange('20200922', '20210118')
