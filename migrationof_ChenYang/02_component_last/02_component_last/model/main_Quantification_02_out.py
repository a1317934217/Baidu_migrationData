...
def get_cities_num(net1, net2,n1,n2):
    ...
    # file = open("D:/workspace/python/test/02_新冠/net_data/city_list_in/"+name1+"_"+name2+"_nodes_list.txt", "w")
    file = open("D:/workspace/python/test/02_新冠/net_data/city_list_out/" + name1 + "_" + name2 + "_nodes_list.txt", "w")
...
def get_single_cities_num(net, name):
    ...
    # file = open("D:/workspace/python/test/02_新冠/net_data/city_list_in/"+name+"_nodes_list.txt", "w")
    file = open("D:/workspace/python/test/02_新冠/net_data/city_list_out/"+name+"_nodes_list.txt", "w")
...
def read_total_cities(strs):
    """读取每个多层网络中的结点数据"""
    # file = open("D:/workspace/python/test/02_新冠/net_data/city_list_in/"+strs+"_nodes_list.txt", "r")
    file = open("D:/workspace/python/test/02_新冠/net_data/city_list_out/"+strs+"_nodes_list.txt", "r")
...
def N_I_J(G):
    ...
    for i in sub_node_list:
        for n in i:
            ...
            # 依次获得任意两节点之间的最短路径
            for target in mm:
                shortest_path_len = nx.shortest_path_length(G, target=n, source=target)
