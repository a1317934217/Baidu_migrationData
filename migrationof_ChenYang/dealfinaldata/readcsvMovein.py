...
for i in range(len(dayList)):
    ...
    with open('../ChinaAreaCodes.csv','r', encoding= 'utf-8', newline='') as csv_chinaCityCode:
        ...
        # 开始写入数据
        with open("F:\\01大连民族\百度迁徙爬取和数据\\21年7月1日开始的前夕数据处理\百度迁徙数据爬取\处理之后的迁徙数据\in\\" + dayList[i] + ".csv", 'w', encoding="utf-8", newline='') as csvfile:
            ...
            for csv_row in chinaCityCode:
                ...
                try:
                    baiduMigrationIndex = pd.read_csv("F:\\01大连民族\百度迁徙爬取和数据\\21年7月1日开始的前夕数据处理\百度迁徙数据爬取\迁徙指数\inneed\\"""+csv_row[0]+"_"+csv_row[1]+"_move_in.csv")
                except Exception as problem:
                    print("error打开迁徙指数有问题：",problem)
                # 循环打开文件 百度迁徙比例 迁入
                # 示例：F:\01大连民族\百度迁徙爬取和数据\21年7月1日开始的前夕数据处理\百度迁徙数据爬取\迁徙比例\in\110000_北京_move_in_20210701.csv
                try:
                    baiduMigrationProportion = pd.read_csv("F:\\01大连民族\百度迁徙爬取和数据\\21年7月1日开始的前夕数据处理\百度迁徙数据爬取\迁徙比例\inneed\\"""+csv_row[0]+"_"+csv_row[1]+"_move_in_"+dayList[i]+".csv")
                except Exception as problem:
